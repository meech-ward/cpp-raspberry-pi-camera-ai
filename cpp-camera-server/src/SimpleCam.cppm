...
using FrameCallback = std::function<void(const Buffer &)>;

ThreadPool pool(4);
export class SimpleCam {
 private:
 ...
 public:
  ...
  SimpleCam(int _width, int _height, int _quality) : height(_height), width(_width), quality(_quality) {
  }
...
void SimpleCam::bufferComplete(Request *request, FrameBuffer *frameBuffer) {
  ...
  // raw_frame_buffer.push(image.clone());  // Clone to ensure data persistence
...
int SimpleCam::start() {
   ...
   */
  // #if 0
  streamConfig.size.width = width;    // 4096
  streamConfig.size.height = height;  // 2560
                                      // streamConfig.stream->
